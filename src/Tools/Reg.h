...
public:
	...
	int ReadString(const wchar_t *name, std::wstring *str);
	int SaveWindowPos(HWND hwnd, const wchar_t *name, int Size = FALSE, int Max = FALSE);	//Restore and save Widht and Height too?  (Not for Dialogs!!!)
	int RestoreWindowPos(HWND hwnd, const wchar_t *name, int Size = FALSE, int Max = FALSE);	//Set MAx to TRUE to save/restore window's maximised state.
};
